...
/*
B MAIN   @xxx 不设置内存linker 0  不行，但为什么呢？ 哦，是不是从0开始，不规定代码在0上，cpu找不到
B .
B SWI_HANDLER
B .
B .
...
MAIN:
	MOV SP, #0x40000020
	MSR CPSR, #0x10   @xxx 执行了这一步，sp怎么变成0了，当然swi后又变回来了
	MOV R1, #1
	MOV R2, #2
	SWI #1
	ADD R3, R1, R2
...
SWI_HANDLER:
	...
	MOV R2, #20
	SUB R3, R2, R1
	LDMFD SP!, {R1, R2, PC}^   @xxx 是不是去掉fd都行？ 哦，之前用的R，LR（之后换CP 不用考虑本函数内有没有调用了）
	                           @xxx pc cp分清！！！
*/
...
.rept 2   @xxx 觉得word rept像单词，align space 像格式 weak也算格式
MOV R1, #1
MOV R2, #2
.endr
...
.byte 0xFF   @xxx byte会造成不对齐，要在之后给个对齐指令！！！
.align 2
MOV R2, #2

.align 4
MOV R2, #2   @xxx 为什么执行完这个指令R1变成0了？
...
MOV R1, #1
MOV R2, #2
BL func_c    @xxx 没有下面的FUNC，跳回来，跳不到下一个指令呢？c语言最后不改pc？
MOV R3, #3

.global FUNC_ASM
FUNC_ASM:    @xxx 应该按照函数调用的4步，时间原因，暂不弄
	MOV R4, #4
	MOV R5, #5
*/

...
int __init mychar_init(void){
	...
	ret = alloc_chrdev_region(&devno, minor, mychar_num, "mychar_sema");
	if (ret != 0) {
	...
	for (i = 0;i < MYCHAR_DEV_CNT; i++) {
		...
		cdev_add(&gmydev[i].mydev, devno, mychar_num);

		init_waitqueue_head(&gmydev[i].rq);
		init_waitqueue_head(&gmydev[i].wq);
		
		sema_init(&gmydev[i].semaphore, 1);
	}
